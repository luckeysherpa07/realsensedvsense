...
FOV_CROP = 0.80  
TILE_HEIGHT = 480 # Increased height for better visibility of 2-pane view
    ...
    try:
        while True:
            ...
            if ir_frame:
                ...
                ir_overlay = cv2.cvtColor(ir_img_gray, cv2.COLOR_GRAY2BGR)
            else:
                ir_overlay = np.zeros((depth_img.shape[0], depth_img.shape[1], 3), dtype=np.uint8)
            ...
            if projector:
                ...
                warped_dvs, fov_mask = projector.project(dvs_raw_clr, depth_tensor, off_x=offset_x, off_y=offset_y, crop_factor=FOV_CROP)
                
                mask_cpu = fov_mask.cpu().numpy().astype(bool)
                warped_u8 = (warped_dvs.permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)
                
                ir_overlay[~mask_cpu] = 0
                warped_u8[~mask_cpu] = 0
                mask_ev_px = np.any(warped_u8 > 10, axis=2)
                if np.any(mask_ev_px):
                    ir_overlay[mask_ev_px] = cv2.addWeighted(ir_overlay[mask_ev_px], 0.5, warped_u8[mask_ev_px], 1.0, 0)

            # --- 5. VISUALIZATION ---
            v_rgb = resize_maintain_aspect(rgb_img, TILE_HEIGHT)
            v_ov  = resize_maintain_aspect(ir_overlay, TILE_HEIGHT)

            # Ensure widths match for a clean side-by-side
            target_w = v_ov.shape[1]
            v_rgb = cv2.resize(v_rgb, (target_w, TILE_HEIGHT))

            add_label(v_rgb, "RGB Camera")
            add_label(v_ov, "IR + Event Overlay")

            # Horizontal side-by-side stack
            display_stack = np.hstack((v_rgb, v_ov))

            cv2.imshow("RealSense + DVS Overlay", display_stack)
